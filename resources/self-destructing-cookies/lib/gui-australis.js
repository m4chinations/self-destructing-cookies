...
exports.setup = function(options) {
  ...
  // the panel we will attach to the button
  sdcpanel = Panel({
    width: 300,
    height: 230,
  ...
  // install a button in the add-on bar
  sdcbutton = ActionButton({
    id: "self-destructing-cookies",
    label: "Self-Destructing Cookies",
    icon: {
      "32": self.data.url("toolbar-w.png")
    },
    onClick: function(state) {
      exports.handleButtonClick();
    }
  });

  // button needs to be updated when users interact with tabs
...
// update buttons when tabs change
exports.handleTabChanged = function(tab) {
  if (!sdcbutton) return;

  if (!armed) {
    sdcbutton.state(tab, {
      icon: self.data.url("toolbar-w.png")
    });
    return;
  }
  ...
  if (host == null) {
    sdcbutton.state(tab, {
      icon: self.data.url("toolbar-r.png")
    });
    return;
  }
  ...
  switch (getDomainWhitelist(host)) {
    ...
    case Ci.nsICookiePermission.ACCESS_ALLOW_FIRST_PARTY_ONLY:
      sdcbutton.state(tab, {
        icon: self.data.url("toolbar-g.png")
      });
      break;
    case Ci.nsICookiePermission.ACCESS_SESSION:
      sdcbutton.state(tab, {
        icon: self.data.url("toolbar-y.png")
      });
      break;
    default:
      sdcbutton.state(tab, {
...
// user clicked the button, prepare the panel
exports.handleButtonClick = function() {
